...
//破壊プレイヤー
constexpr float BREAK_SCALE = 0.4f;
...
Player::~Player()
{
	...
	DeleteEffekseerEffect(spark_.resource);
	magStick_->Release();
	//DeleteGraph();
}
...
bool Player::Init(Vector3 pos)
{
	...
	//エフェクト
	spark_.resource			= LoadEffekseerEffect("./source/effect/spark.efkefc", EFFECT_SPARK);
	spark_.playhandle		= -1;
...
{
	//磁石の棒更新
	magStick_->Update(GimmickList_, unitlist_, Boxlist_, pos_, moveVecPow_ - moveVecPowMag_, state_, input_->GetCntData(), enemy);
	//磁石の棒の角度更新
	...
	else
	{
		if (state_ != State::Magnet)
		{
			if (state_ != State::MagnetWeak)
			{
				if (state_ != State::Fall)
				{
					if (abs(Playerbody_->getInterpolationLinearVelocity().x()) > KEY_MOVE_SPARK_ON_LIMIT)
					{
						spark_.playhandle = PlayEffekseer3DEffect(spark_.resource);
					}
				}
...
{
	spark_.pos = pos_;
	SetPosPlayingEffekseer3DEffect(spark_.playhandle, spark_.pos.x, spark_.pos.y, spark_.pos.z);

	//if (IsEffekseer3DEffectPlaying(trail_.playhandle) == 0)
	{
		trail_.pos = pos_;
		SetPosPlayingEffekseer3DEffect(trail_.playhandle, trail_.pos.x, trail_.pos.y, trail_.pos.z);
	}
