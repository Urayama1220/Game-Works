...
constexpr double SPEEDDOWN_MAX_TIME = static_cast<double>(MOVE_SPEED * DIRECTION_DIVISION) / 60.0;
...
bool Bullet::Init(Vector3 direction, Vector3 enemypos)
{
	...
	mat = MMult(mat, MGetScale(VGet(3.0f, 3.0f, 3.0f)));
...
void Bullet::DownSpeed(void)
{
	if(!inversionFlag_)
	{
		//移動スピードダウン
		movePower_ -= (direction_ / DIRECTION_DIVISION);
		stepSpeedDownTime += lpSceneMng.GetDeltaTime();
		if (stepSpeedDownTime > SPEEDDOWN_MAX_TIME)
		{
			inversionFlag_ = true;
			auto vec = atan2(-direction_.y, -direction_.x);
