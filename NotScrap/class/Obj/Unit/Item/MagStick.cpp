...
void MagStick::Update(GimmickList& gimmicklist, EneymyList& enemyList, MagBoxList& boxList, Vector3 pos, Vector3 dir,State state,const CntData& cntdata, const EneymyList& enemy)
{
	//敵情報取得
	...
	if (cntType_ == CntType::Pad)
	{
			...
			(abs(rStick_Input.Rx) >= LOWER_LIMIT || abs(rStick_Input.Ry) >= LOWER_LIMIT))
		{
			...
			vec.Normalize();
			Vector2 plusVec = vec * ROD_LENGTH;
			auto screenPos = ConvWorldPosToScreenPos(VGet(line_.p.x,line_.p.y,line_.p.z));
			auto totalWorldPos = ConvScreenPosToWorldPos(VGet(screenPos.x + plusVec.x, screenPos.y + plusVec.y, screenPos.z));
			//マウスの位置更新
		...
		{
			//傾けていない場合、プレイヤーの位置から前フレームの傾きを足す
			auto screenPos = ConvWorldPosToScreenPos(VGet(line_.p.x, line_.p.y, line_.p.z));
			auto totalWorldPos = ConvScreenPosToWorldPos(VGet(screenPos.x + plusPos_.x, screenPos.y + plusPos_.y, screenPos.z));
			endPos_.x = totalWorldPos.x;
			endPos_.y = totalWorldPos.y;
		}
	}
	...
	MV1SetRotationXYZ(lpModelMng.GetID("./source/model/Player/Chocolate-Stick.mv1")[num_], { 0, 0/*AsoUtility::Deg2RadF(-90.0f)*/, (float)angle_ });
	...
	MV1SetRotationXYZ(lpModelMng.GetID("./source/model/Player/Stick.mv1")[num_], { 0, 0/*AsoUtility::Deg2RadF(-90.0f)*/, (float)angle_ });
	

	//各オブジェクトとの判定
	CheckMagnetColl();
	CheckMagnetBox(gimmicklist);
	CheckAccessory(gimmicklist);
	CheckStickEnemy();

	//軌跡の更新
	trail_->Update(line_,state_);
}
...
void MagStick::Draw(void)
{
	...
	//MV1DrawModel(lpModelMng.GetID("./source/model/Player/Stick.mv1")[num_]);
...
void MagStick::CheckMagnetColl(void)
{
	playerMoveVec_ = { 0, 0 ,0 };
	...
	{
		v = ConvertVECTORtoVector3(ConvScreenPosToWorldPos(VGet(trail_->GetCenterPos().x, trail_->GetCenterPos().y, 1.0f)));
	}
...
void MagStick::CheckMagnetBox(GimmickList& gimmicklist)
{
		...
		if (magbox->GetObjType() == objType::NMagBox)
		{
				...
				CollisionRay()(leftLine_, node, lpModelMng.GetID("./source/Model/Magnet/N_Box.mv1")[magbox->GetModelH()]).HitFlag)
			{
				...
				if (state_ == MAGSTATE::North)
				{
					...
					{
						magbox->GetRigidBody()->applyCentralForce(normal_.tobtVec()*50000000);
...
void MagStick::CheckAccessory(GimmickList& gimmicklist)
{
	auto node = stateNode_->first_node("module");
	for (auto& acs : gimmicklist)
	{
		...
		if (acs->GetObjType() == objType::Accessory)
		{
				...
				CollisionRay()(leftlineAcs, node, lpModelMng.GetID("./source/Model/Stage/Barrel2.mv1")[acs->GetModelH()]).HitFlag)
			{
				if (state_ == MAGSTATE::North)
				{
					acs->GetRigidBody()->applyCentralImpulse(normal_.tobtVec() * 15);
...
void MagStick::CheckStickEnemy(void)
{
	auto node = stateNode_->first_node("module");
...
void MagStick::SetLineInfo(void)
{
	...
	{
		v = ConvertVECTORtoVector3(ConvScreenPosToWorldPos(VGet(960.0f, 670.0f, 1.0f)));
	}
	...
	if (d == Vector3{ 0.0,0.0,0.0 })
	{
		d = Vector3{ 1.0,1.0,0.0 };
	}
	...
	//正規化
	normal_ = d.Normalized();

	//真ん中の線分の位置更新
	line_.end = line_.p + normal_ * rodLength_;
	line_.end.z = 0.0f;
	...
	//左の線分の位置更新
	addx = cos(q - deg90) * 30.0f;
	addy = sin(q - deg90) * 30.0f;
	pos_.x = line_.p.x + addx;
	pos_.y = line_.p.y + addy;
