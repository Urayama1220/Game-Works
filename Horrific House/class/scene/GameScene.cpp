...
UniqueScene GameScene::Update(UniqueScene scene)
{
	...
	stage_->Update();
	//カメラ
	camera_->Update(player_->GetDashFlag(),player_->GetOpenDoorFlag());
	//プレイヤー
	player_->GetItemID();
	player_->Update(camera_->GetAngles(), camera_->GetTargetPos());
	//敵
	enemy_->Update(player_->GetCapsule(), player_->GetPosition(), player_->CheckHitFlash(camera_->GetTargetPos()),player_->GetItemID(),stage_->GetGimmickList());
	//低感度状態がtureだったら
...
void GameScene::DrawScreen(void)
{
	...
	//ステージ
	stage_->Draw(player_->GetIsOnCamera(),depthRT_);
	//敵
	enemy_->Draw();
	//プレイヤー
