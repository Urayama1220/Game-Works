...
void MinCamera::Init(void)
{
	...
	//シェーダ関係初期化
	int vs = LoadVertexShader("resource/shader/vs/MinCamera_vs.vso");
	int ps = LoadPixelShader("resource/shader/ps/MinCamera_ps.pso");
...
void MinCamera::Draw(void)
{
	if (onCamera_)
	{
		...
		//origShader_->Draw(lpModelMng.GetID("resource/model/smh.mv1")[0], lpImageMng.GetID("resource/model/texture/smhfile/smartphone_s.tga")[0], lpImageMng.GetID("resource/model/texture/smhfile/smartphone_n.tga")[0]);
		minCameraUI_->Draw();
	}
}
