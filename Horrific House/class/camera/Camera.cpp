...
void Camera::Update(bool dashFlag,bool openDoorFlag)
{
	//if (openDoorFlag)
	//{
	//	targetPos_.y += SinCurve(200.0f, sinCount_, 100.0f);
	//	sinCount_ += lpSceneMng.GetDeltaTime();
	//	return;
	//}
	if (GetJoypadNum())
	{
...
{
	return (waitTime_ += lpSceneMng.GetDeltaTime()) > waittime;//指定した待ち時間が経過したらtrueを返す。
}